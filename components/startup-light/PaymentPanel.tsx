...
interface PaymentPanelProps {
  ...
  /**
   * ECPay form HTML from /api/create-payment.
   * Only present when PAYMENT_PROVIDER=newebpay.
   *
   * Safety: formHtml MUST come from the server API, never from user input.
   * It is injected via dangerouslySetInnerHTML only because the form must be
   * submitted to the ECPay gateway (cross-origin POST with hidden fields).
   * React does not execute <script> inside dangerouslySetInnerHTML, so the form
   * is submitted manually via containerRef + useEffect + form.submit().
   */
...
export function PaymentPanel({ showPayment, paymentData, paymentConfirmed, paymentLoading, confirmLoading, boundaryError, analysisData, formHtml, onPaymentClick, onConfirmPayment }: PaymentPanelProps) {
  ...
  /**
   * Submit the NewebPay form upon receiving formHtml.
   *
   * React dangerouslySetInnerHTML does NOT execute <script> tags, so the
   * auto-submit script inside formHtml cannot run. We inject the HTML into
   * a hidden container and manually call form.submit() here.
   *
   * hasSubmittedRef prevents duplicate submissions if formHtml changes
   * (React strict mode double-render, etc.).
   */
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (form) {
        ...
        console.warn("[PaymentPanel] formHtml injected but no <form> found in container");
      }
    }, 100);
  ...
  return (
    <>
      ...
      {(paymentData && !paymentConfirmed && !analysisData && formHtml) ? (
        <section className="mb-8 rounded-2xl border border-border-subtle bg-gradient-to-br from-bg-card to-bg-card/60 p-6 backdrop-blur-sm sm:p-8">
          <div className="mx-auto mb-4 inline-flex items-center gap-2 rounded-full border border-blue-light/30 bg-blue-light/10 px-4 py-1.5 text-sm font-semibold text-blue-light">綠界金流</div>
          <h2 className="mb-2 text-lg font-bold text-white">正在前往綠界付款頁…</h2>
        ...
        <section className="mb-8 rounded-2xl border border-border-subtle bg-gradient-to-br from-bg-card to-bg-card/60 p-6 backdrop-blur-sm sm:p-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-yellow-light/30 bg-yellow-light/10 px-4 py-1.5 text-sm font-semibold text-yellow-light">首次檢查 49 元</div>
          <p className="mb-4 text-sm leading-relaxed text-text-secondary">點擊下方按鈕前往付款，完成後即可開始填寫完整判定資料。</p>
      ...
      {/*
      Hidden formHtml container for ECPay form.

      formHtml is generated by create-payment route using server-side env vars
      (HashKey, HashIV) — never from user input. The form is rendered hidden
      and submitted programmatically via the useEffect above.

      dangerouslySetInnerHTML is acceptable here because:
      1. formHtml originates from our own API, not user input
      2. The content is a static form with hidden fields, no interactive scripts
      3. React strips <script> tags anyway — we handle submission ourselves
      */}
      {formHtml && (
