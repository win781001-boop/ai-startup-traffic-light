...
- Public Beta ????? 6 ?????? normalize ? sha256 hash
  (trim + lowercase + ????? + ???? + sha256)
- ?? hash 24 ???????? 1 ?
- Key: `beta:submission-hash:YYYY-MM-DD:<hash>`
- ?????? 409 `duplicate_beta_submission`???? payment???? Tavily/AI
- 2026-06-17 incident ??? payload ??? 614 ??? guard ? P0
- Production ???? Upstash?memory fallback ? serverless ? instance ????


**Public Beta Duplicate Payload Guard (P0)**
- Public Beta ????? 6 ?????? normalize ? sha256 hash
  ?trim + lowercase + ????? + ?????? `|` ??? sha256?
- ?? hash 24 ???????? 1 ?
- Key: `beta:submission-hash:YYYY-MM-DD:<hash>`
- ???????? 409 `duplicate_beta_submission`???? payment ? submission
- 2026-06-17 incident ??? payload ??? 614 ????? guard ? P0 ??
- ???? Upstash Redis?? Upstash ? memory fallback ??? serverless ? instance
- ?????field validation ? duplicate payload guard ? beta auto-create payment ? (??????)
